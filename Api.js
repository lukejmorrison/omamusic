...
function albumShareUrl(item) {
  if (!item || typeof item !== "object") return ""
  var url = String(item.externalUrl || "").trim()
  if (url) return url
  var playlistId = String(item.playlistId || "").trim()
  if (playlistId) return "https://music.youtube.com/playlist?list=" + playlistId
  var browseId = String(item.id || item.browseId || "").trim()
  return browseId ? "https://music.youtube.com/browse/" + browseId : ""
}
...
function collectionCountLabel(count, filterText, loading) {
...
function isAlbumBrowseId(value) {
  return String(value || "").indexOf("MPRE") === 0
}

function isArtistId(value) {
  return String(value || "").indexOf("UC") === 0
}
...
function isPlaylistId(value) {
  ...
  return /^(PL|OL|RD|VL)/.test(id)
}

function contextPlayableTracks(item) {
  var source = arrayValues(item && item.tracks)
  var out = []
  for (var i = 0; i < source.length; i++) {
    if (source[i] && source[i].type === "track" && videoIdOf(source[i]))
      out.push(source[i])
  }
  return out
}

function detailRequest(item) {
  if (!item || typeof item !== "object") return null
  var ident = String(item.id || "").trim()
  var playlistId = String(item.playlistId || "").trim()
  var type = String(item.type || "")
  if (type === "artist" || isArtistId(ident))
    return ident ? { command: "get_artist", item_id: ident } : null
  if (type === "album") {
    if (isAlbumBrowseId(ident))
      return { command: "get_album", item_id: ident }
    if (playlistId)
      return { command: "get_playlist", item_id: playlistId }
    if (isPlaylistId(ident))
      return { command: "get_playlist", item_id: ident }
    if (ident)
      return { command: "get_album", item_id: ident }
    return null
  }
  var playlistIdent = ident || playlistId
  return playlistIdent ? { command: "get_playlist", item_id: playlistIdent } : null
}

function playLoadFields(item) {
  if (!item || typeof item !== "object") return null
  var ident = String(item.id || "").trim()
  var playlistId = String(item.playlistId || "").trim()
  var type = String(item.type || "")
  if (type === "artist" || isArtistId(ident))
    return ident ? { artist_id: ident } : null
  if (type === "album") {
    if (isAlbumBrowseId(ident)) return { album_id: ident }
    if (playlistId) return { playlist_id: playlistId }
    if (isPlaylistId(ident)) return { playlist_id: ident }
    if (ident) return { album_id: ident }
    return null
  }
  if (type === "playlist" || item.kind === "context") {
    var playlistIdent = ident || playlistId
    return playlistIdent ? { playlist_id: playlistIdent } : null
  }
  return null
}
