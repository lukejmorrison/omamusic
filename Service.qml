...
Item {
  ...
  readonly property string currentAlbumId: currentAlbumItem
    ? String(currentAlbumItem.id || currentAlbumItem.playlistId || "") : ""
  readonly property string currentAlbumPlaylistId: currentAlbumItem
    ? String(currentAlbumItem.playlistId || currentAlbumItem.id || "") : ""
  property bool currentAlbumSaved: false
  onCurrentAlbumIdChanged: currentAlbumSaved = !!(currentAlbumItem
    && currentAlbumItem.liked === true)
  ...
  property int detailRequestGen: 0
  ...
  function playItem(item, sourceItems, contextUri) {
    ...
    if (item.kind === "context" || (item.type && item.type !== "track")) {
      openDetail(item)
      var nested = Api.contextPlayableTracks(item)
      if (nested.length) {
        command("load", { items: nested, index: 0 }, "Playing " + (item.name || "selection"))
        return
      }
      var fields = Api.playLoadFields(item)
      if (!fields) {
  ...
  function toggleCurrentAlbumSaved() {
    var playlistId = currentAlbumPlaylistId
    if (!playlistId) {
      fail("No album to like")
      return
    }
    if (!accountConnected) {
      fail("Sign in to like albums")
      login()
      return
    }
    var liked = !currentAlbumSaved
    command("like", { playlist_id: playlistId, liked: liked },
      liked ? "Liked album" : "Removed album like",
      function(ok) {
        if (ok) root.currentAlbumSaved = liked
      })
  }
  ...
  function copyAlbumLink(item) {
    var album = item && item.type === "album" ? item
      : (item && item.albumItem ? item.albumItem : currentAlbumItem)
    var url = Api.albumShareUrl(album)
    if (!url) {
      fail("No album link")
      return
    }
    Quickshell.execDetached(["bash", "-c", "printf %s " + Util.shellQuote(url) + " | wl-copy"])
    succeed("Copied album link")
  }
  ...
  function openDetail(item) {
    ...
    detailAlbums = []
    detailLoading = true
    var requestGen = ++detailRequestGen
    var request = Api.detailRequest(item)
    if (!request) {
      detailLoading = false
      detailWatch.stop()
    ...
    detailWatch.restart()
    command(request.command, { item_id: request.item_id }, "", function(ok, result) {
      if (requestGen !== root.detailRequestGen) return
      root.detailWatch.stop()
  ...
  BackendClient {
    ...
    onSpectrumReceived: function(bands) {
  ...
  Timer {
    ...
    repeat: false
    onTriggered: {
      if (!root.detailLoading) return
      root.detailLoading = false
      root.fail("That page took too long to load. Try again.")
    }
  }
