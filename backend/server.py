...
class Backend:
    ...
    def _restore_queue_session(self) -> None:
        ...
        if items:
            ...
            self._resume_playing = bool(session.get("playing"))
            return
    ...
    def _resume_queue_session(self) -> None:
        if not self._resume_playing or not self.player.current:
            return
        self._resume_playing = False
        try:
            self.player.play()
    ...
    def dispatch(self, command: str, message: dict[str, Any]) -> dict[str, Any]:
        ...
        if command == "get_playlist":
            with self._catalog_lock:
                self._refresh_session()
                return self.require_catalog().playlist(str(
                    message.get("item_id") or message.get("playlist_id") or ""))
        if command == "get_album":
            with self._catalog_lock:
                self._refresh_session()
                return self.require_catalog().album(str(
                    message.get("item_id") or message.get("album_id") or ""))
        if command == "get_artist":
            with self._catalog_lock:
                self._refresh_session()
                return self.require_catalog().artist(str(
                    message.get("item_id") or message.get("artist_id") or ""))
        if command == "get_queue":
            return {"items": list(self.player.queue), "index": self.player.index}
        if command == "like":
            playlist_id = str(message.get("playlist_id") or message.get("album_id") or "")
            if playlist_id:
                return self.like_playlist(playlist_id, bool(message.get("liked")))
    ...
    def like_playlist(self, playlist_id: str, liked: bool) -> dict[str, Any]:
        if not playlist_id:
            raise ValueError("playlist_id is required")
        if not self.signed_in:
            raise AuthError("Sign in to like albums")
        try:
            self.require_catalog().rate_playlist(playlist_id, liked)
        except AuthRequired as exc:
            self._invalidate_session()
            raise AuthError(str(exc)) from exc
        current = self.player.current
        album = (current or {}).get("albumItem")
        if isinstance(album, dict) and (
                album.get("playlistId") == playlist_id or album.get("id") == playlist_id):
            album["liked"] = liked
        self.broadcast()
        return {"liked": liked, "playlist_id": playlist_id}
    ...
    def load(self, message: dict[str, Any]) -> dict[str, Any]:
        ...
        if isinstance(items, list) and items:
            ...
            if str(album_id).startswith("MPRE"):
                resolved = self.require_catalog().album(album_id).get("tracks") or []
            else:
                resolved = self.require_catalog().playlist(album_id).get("tracks") or []
        elif artist_id:
    ...
    def serve(self, path: Path) -> None:
        ...
        spectrum_thread = threading.Thread(target=self._spectrum_watch, daemon=True)
    ...
    def _spectrum_watch(self) -> None:
        ...
        while not self._stop.is_set():
            ...
            if last and max(abs(a - b) for a, b in zip(bands, last)) < 0.02:
                last = bands
                continue
            last = bands
