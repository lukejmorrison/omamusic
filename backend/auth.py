...
def refresh_browser_authorization(path: Path) -> None:
    """Recompute SAPISIDHASH from the stored cookies.

    YouTube's hash is time-based. A snapshot written at import time goes
    stale within a day and looks like “sign in again” even though the
    ...
    for key in list(headers):
        if str(key).lower() == "authorization":
            auth_key = key
            break
    headers[auth_key] = authorization
...
def _ensure_browser_authorization(path: Path) -> None:
    refresh_browser_authorization(path)
