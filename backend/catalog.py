...
def album_item(item: dict | None) -> dict | None:
    ...
    if not isinstance(album, dict):
        return None
    browse_id = _text(album.get("browseId") or album.get("id"))
    playlist_id = _text(album.get("audioPlaylistId") or album.get("playlistId"))
    if _looks_like_playlist(browse_id) and not _looks_like_album_browse(browse_id):
        playlist_id = playlist_id or browse_id
        browse_id = ""
    name = _text(album.get("name") or album.get("title"))
    if not name and not browse_id and not playlist_id:
        return None
    return {
        "kind": "context",
        "type": "album",
        "id": browse_id,
        "uri": album_uri(browse_id) or playlist_uri(playlist_id),
        "name": name or "Album",
        "subtitle": artist_names(source),
        "imageUrl": thumbnail_url(album) or thumbnail_url(source),
        "playlistId": playlist_id,
        "externalUrl": browse_url(browse_id) or playlist_url(playlist_id),
    }


def attach_album_context(track: dict, parent: dict) -> None:
    """Fill a track's album stub with the parent album browse/playlist ids."""
    if not isinstance(track, dict) or not isinstance(parent, dict):
        return
    current = track.get("albumItem") if isinstance(track.get("albumItem"), dict) else {}
    merged = dict(current)
    for key in ("id", "uri", "name", "playlistId", "imageUrl", "subtitle"):
        if not _text(merged.get(key)) and _text(parent.get(key)):
            merged[key] = parent[key]
    merged["kind"] = "context"
    merged["type"] = "album"
    if not _text(merged.get("uri")):
        merged["uri"] = album_uri(_text(merged.get("id"))) or playlist_uri(
            _text(merged.get("playlistId")))
    track["albumItem"] = merged
    if not _text(track.get("album")):
        track["album"] = _text(merged.get("name") or parent.get("name"))
    if not _text(track.get("imageUrl")) and _text(parent.get("imageUrl")):
        track["imageUrl"] = parent["imageUrl"]
...
def _looks_like_album_browse(value: Any) -> bool:
    return _text(value).startswith("MPRE")


def _looks_like_playlist(value: Any) -> bool:
    text = _text(value)
    return text == "LM" or text.startswith(("PL", "OL", "RD", "VL"))
...
class Catalog:
    ...
    def history(self, limit: int = 40) -> list[dict]:
        try:
            ...
            if looks_unauthorized(exc):
                raise_auth_or_catalog(exc, "see history")
            raw = []
    ...
    def liked(self, limit: int = 50) -> list[dict]:
        try:
            ...
            raise_auth_or_catalog(exc, "see liked songs")
        return map_items(raw, limit=limit)
    ...
    def playlists(self, limit: int = 50) -> list[dict]:
        try:
            ...
            raise_auth_or_catalog(exc, "see playlists")
    ...
    def library_songs(self, limit: int = 50) -> list[dict]:
        try:
            ...
            raise_auth_or_catalog(exc, "see your library")
        return map_items(raw, limit=limit)
    ...
    def playlist(self, playlist_id: str, limit: int = 80) -> dict:
        ...
        if not _text(header.get("id")):
            header["id"] = playlist_id
        header["playlistId"] = _text(header.get("playlistId")) or playlist_id
        header["uri"] = playlist_uri(_text(header.get("id"))) or playlist_uri(playlist_id)
        header["externalUrl"] = header.get("externalUrl") or playlist_url(
            _text(header.get("id")) or playlist_id)
    ...
    def album(self, album_id: str) -> dict:
        ...
        playlist_id = _text(
            raw.get("audioPlaylistId") or raw.get("playlistId") or header.get("playlistId"))
        if _looks_like_album_browse(album_id):
            header["id"] = album_id
            header["uri"] = album_uri(album_id)
            header["externalUrl"] = browse_url(album_id) or header.get("externalUrl") or ""
        elif not _text(header.get("id")):
            header["id"] = album_id
        header["playlistId"] = playlist_id
        parent_album = {
            "kind": "context",
            "type": "album",
            "id": _text(header.get("id")),
            "uri": _text(header.get("uri")) or album_uri(_text(header.get("id"))),
            "name": _text(header.get("name")),
            "playlistId": playlist_id,
            "imageUrl": _text(header.get("imageUrl")),
            "subtitle": _text(header.get("subtitle")),
        }
    ...
    def rate_playlist(self, playlist_id: str, liked: bool) -> None:
        rating = "LIKE" if liked else "INDIFFERENT"
        try:
            self.yt.rate_playlist(playlist_id, rating)
        except Exception as exc:
            raise_auth_or_catalog(exc, "like albums")
