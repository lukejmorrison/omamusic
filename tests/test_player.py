...
class PlayerTests(unittest.TestCase):
    ...
    def test_mpv_command_stays_headless(self):
        ...
        self.assertIn("--audio-client-name=omamusic", command)
    ...
    def test_playback_error_message_rewrites_youtube_refusals(self):
        self.assertEqual(
...
class QueueAndEqTests(unittest.TestCase):
    ...
    def test_restore_queue_sets_current_without_playing(self):
        with tempfile.TemporaryDirectory() as tmp:
            qp = QueuePlayer(Path(tmp))
            qp.restore_queue(
            ...
            self.assertEqual(qp.current["videoId"], "bbb")
            self.assertFalse(qp.playing)
            self.assertTrue(qp.shuffle)
            self.assertEqual(qp.repeat, "track")
            self.assertEqual(qp.snapshot_track()["name"], "Two")

    def test_play_loads_a_restored_track(self):
        from unittest import mock
        with tempfile.TemporaryDirectory() as tmp:
            qp = QueuePlayer(Path(tmp))
            qp.restore_queue([{"videoId": "aaa", "name": "One"}])
            with mock.patch.object(qp, "_play_current") as play_current:
                qp.play()
            play_current.assert_called_once_with(start=True)

    def test_play_seeks_to_the_saved_position(self):
        from unittest import mock
        with tempfile.TemporaryDirectory() as tmp:
            qp = QueuePlayer(Path(tmp))
            qp.restore_queue(
            ...
            with mock.patch.object(qp, "_play_current"), \
                 mock.patch.object(qp, "seek") as seek:
                qp.play()
            seek.assert_called_once_with(15000)
