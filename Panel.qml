...
Item {
  ...
  readonly property string pluginId: manifest && manifest.id
    ? String(manifest.id) : "wizwam.omamusic"
  ...
  function likeCurrentAlbum() {
    ...
    service.toggleCurrentAlbumSaved()
  }
  ...
  Timer {
    interval: 250
    running: root.opened && root.currentTab === "login" && root.accountConnected
    repeat: false
    onTriggered: root.updateLoginGate()
  }
  ...
  Popup {
    ...
    dim: true
    z: 1000
    x: Math.round((focusScope.width - width) / 2)
    y: Math.round((focusScope.height - height) / 2)
  ...
  Component {
    ...
    Flickable {
      clip: true
      // A short form inside a tall Flickable eats button presses on Qt 6.
      interactive: false
      ...
      Column {
        ...
        Text {
          width: parent.width
          visible: !root.accountConnected && !!(root.service && root.service.loginProgress)
          text: root.service ? root.service.loginProgress : ""
        ...
        Row {
          spacing: Style.space(8)
          Button {
            text: root.service && root.service.authBusy ? "Working…" : "Use Chromium session"
            iconText: "󰍂"
            selected: true
            foreground: root.foreground
            enabled: true
            onClicked: {
              if (!root.service) return
              root.service.importBrowserAuth()
            }
          }
          Button {
    ...
    Flickable {
      ...
      Column {
        ...
        Button {
          ...
          foreground: root.foreground
          onClicked: {
            root.draftShortcutPlayer = root.draftShortcutPlayer === "Omarchy Music app"
              ? "Full player"
              : (root.draftShortcutPlayer === "Full player" ? "Mini player" : "Omarchy Music app")
            root.persistDraftSettings()
          }
        }
        Button {
        ...
        Text {
          width: parent.width
          text: "Realtime frequency from the sound leaving this computer. Same ten bands as cliamp."
          color: Qt.darker(root.foreground, 1.45)
          wrapMode: Text.WordWrap
          font.pixelSize: Style.font.caption
        }
    ...
    FocusScope {
      ...
      Item {
        ...
        Row {
          ...
          BorderSurface {
            ...
            Loader {
              ...
              anchors.topMargin: Style.space(8)
              anchors.bottom: parent.bottom
              sourceComponent: root.currentPage
            }
          }
        }
        ...
        BorderSurface {
          ...
          anchors.right: parent.right
          anchors.bottom: parent.bottom
          height: visible ? Style.space(root.compactHeight ? 228 : 268) : 0
          radius: root.areaRadius
          color: Style.normalFillFor(root.foreground, root.accent)
          borderSpec: Border.none()
          ...
          Item {
            ...
            Artwork {
              ...
              anchors.left: parent.left
              anchors.top: parent.top
              width: Math.max(Style.space(72), Math.min(
                Style.space(root.compactHeight ? 110 : 148),
                Math.max(Style.space(72), playerRow.height - titleColumn.implicitHeight
                  - Style.space(8))))
              height: width
              radius: root.areaRadius
              foreground: root.foreground
            ...
            Column {
              ...
              Item {
                ...
                Text {
                  ...
                  font.family: root.fontFamily
                  font.bold: true
                  wrapMode: root.mediumWidth ? Text.Wrap : Text.NoWrap
                  maximumLineCount: root.mediumWidth ? 2 : 1
                  elide: Text.ElideRight
                ...
                MouseArea {
                  ...
                  cursorShape: root.service && root.service.currentTrackItem
                    ? Qt.PointingHandCursor : Qt.ArrowCursor
                  onClicked: {
                    if (!root.service || !root.service.currentTrackItem) return
                    var track = root.service.currentTrackItem
                    if (track.albumItem) root.openItem(track.albumItem)
                    else if (track.artists && track.artists.length)
                      root.openItem(track.artists[0])
                  }
                }
                PanelToolTip {
              ...
              Flow {
                ...
                spacing: Style.space(6)
                ArtistLinks {
                  width: root.mediumWidth ? titleColumn.width : implicitWidth
                  wrapMode: root.mediumWidth ? Text.Wrap : Text.NoWrap
                  maximumLineCount: root.mediumWidth ? 2 : 1
                  elide: Text.ElideRight
                  artists: root.service && root.service.currentTrackItem
                    ? Api.arrayValues(root.service.currentTrackItem.artists) : []
                  fallbackText: root.service ? String(root.service.artist || "") : ""
                  fallbackClickable: true
                  color: Qt.darker(root.foreground, 1.4)
                  accent: root.accent
                  font.family: root.fontFamily
                  font.pixelSize: Style.font.bodySmall
                  onArtistRequested: function(item) { root.openItem(item) }
                  onFallbackRequested: if (root.service && root.service.currentTrackItem)
                    root.openItem(root.service.currentTrackItem.parentContext
                      || root.service.currentTrackItem)
                }
                ArtistLinks {
                  width: root.mediumWidth ? titleColumn.width : implicitWidth
                  visible: !!(root.service && root.service.currentTrackItem
                    && root.service.currentTrackItem.albumItem)
                  wrapMode: root.mediumWidth ? Text.Wrap : Text.NoWrap
                  maximumLineCount: root.mediumWidth ? 2 : 1
                  elide: Text.ElideRight
                  artists: []
                  fallbackText: visible && root.service.currentTrackItem.albumItem
                    ? String(root.service.currentTrackItem.albumItem.name || "") : ""
                  fallbackClickable: true
                  color: root.accent
                  accent: root.accent
                  font.family: root.fontFamily
                  ...
                  onFallbackRequested: if (root.service && root.service.currentTrackItem
                      && root.service.currentTrackItem.albumItem)
                    root.openItem(root.service.currentTrackItem.albumItem)
                }
              }
            }
            ...
            Column {
              id: transportBox
              anchors.left: artworkNowPlaying.right
              anchors.right: volumeBox.left
              anchors.leftMargin: Style.space(10)
              anchors.rightMargin: Style.space(10)
              anchors.top: parent.top
              anchors.bottom: titleColumn.top
              ...
              Row {
                ...
                Chicklet {
                  id: likeAlbumButton
                  visible: root.service && !!root.service.currentAlbumItem
                  iconText: "󰀥"
                  selected: root.service && root.service.currentAlbumSaved
                  foreground: root.foreground
                  tooltipText: root.service && !root.service.accountConnected
                    ? "Sign in to like albums"
                    : (root.service && root.service.currentAlbumSaved
                      ? "Remove album like" : "Like this album")
                  onClicked: root.likeCurrentAlbum()
                }
                Chicklet {
                  id: shareAlbumButton
                  visible: root.service && Api.albumShareUrl(root.service.currentAlbumItem) !== ""
                  iconText: "󰌹"
                  foreground: root.foreground
                  tooltipText: "Copy album link"
                  onClicked: if (root.service) root.service.copyAlbumLink()
                }
              }
              Row {
                anchors.left: parent.left
              ...
              SpectrumBar {
                ...
                width: parent.width
                compact: true
                visible: root.service && root.service.hasPlayer
                levels: root.service ? root.service.spectrumBands : []
                foreground: root.foreground
                accent: root.accent
              }
              Column {
                width: footerSpectrum.width
                spacing: Style.space(2)
                PlaybackSlider {
                  width: parent.width
                  bar: root.panelBar
                ...
                Row {
                  ...
                  Text {
                    ...
                    Accessible.name: "Elapsed time " + elapsedTime.text
                  }
                  Item {
                    width: Math.max(0, parent.width - elapsedTime.implicitWidth
                      - remainingTime.implicitWidth)
                    height: 1
                  }
                  Text {
                    id: remainingTime
                    ...
                    font.pixelSize: Style.font.caption
                    Accessible.role: Accessible.StaticText
                    Accessible.name: "Duration " + remainingTime.text
                  }
                }
              }
              Text {
